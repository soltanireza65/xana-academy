...
if ( ! class_exists( 'CSF_Welcome' ) ) {
  class CSF_Welcome{
    ...
    public function add_about_menu() {
      add_management_page( 'Codestar Framework', 'فریمورک کُداستار', 'manage_options', 'csf-welcome', array( $this, 'add_page_welcome' ) );
    }
    ...
    public static function add_plugin_action_links( $links, $plugin_file ) {
      ...
      if ( $plugin_file === 'codestar-framework/codestar-framework.php' && ! empty( $links ) ) {
        $links['csf--welcome'] = '<a href="'. esc_url( admin_url( 'tools.php?page=csf-welcome' ) ) .'">تنظیمات</a>';
        if ( ! CSF::$premium ) {
    ...
    public static function add_plugin_row_meta( $links, $plugin_file ) {
      ...
      if ( $plugin_file === 'codestar-framework/codestar-framework.php' && ! empty( $links ) ) {
        $links['csf--docs'] = '<a href="http://codestarframework.com/documentation/" target="_blank">مستندات افزونه</a>';
      }
